...
@property (nonatomic,strong) NSMutableArray *errorModelArr;
@end
...
-(void)requestData{
    ...
    NSString *urlStr = [API_HOST stringByAppendingFormat:DEVICE_FAULT_LIST];
    [Header requestDataWithURLOfPost:urlStr WithParaDic:paramDic WithSucess:^(id result) {
        ...
        if(code == 0){
            ...
            NSArray *modelArr = [ErrorListModel mj_objectArrayWithKeyValuesArray:list];
            [self.errorModelArr addObjectsFromArray:modelArr];
...
-(void)initContentView{
    ...
    self.contentView.chooseBlock = ^(NSDictionary *searchDic){
        ...
        [weakSelf.errorModelArr  removeAllObjects];
...
- (CustomGifHeader *)gifHeader {
    if (!_gifHeader) {
        _gifHeader = [CustomGifHeader headerWithRefreshingBlock:^{
            self.pageIndex = 1;
            [self.errorModelArr removeAllObjects];
            [self requestData];
        }];
    }
    return _gifHeader;
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    return self.errorModelArr.count;
}
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ListTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"listCell" forIndexPath:indexPath];
    ErrorListModel *model = self.errorModelArr[indexPath.row];
    [cell setCustomViewsWithErrorModel:model];
    return cell;
}
...
-(NSMutableArray *)errorModelArr{
    if(!_errorModelArr){
        _errorModelArr = [[NSMutableArray alloc]init];
    }return _errorModelArr;
}
