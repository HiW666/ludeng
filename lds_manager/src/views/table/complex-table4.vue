<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-select v-model="listQuery.role" placeholder="角色类型" clearable class="filter-item" style="width: 130px">
    ...
    >
      ...
      <el-table-column label="用户名字" width="100px">
        <template slot-scope="{row}">
          <span class="link-type" @click="handleUpdate(row)">{{ row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="用户账号" class-name="status-col" width="150">
        <template slot-scope="{row}">
      ...
      <el-table-column label="用户角色" width="110px" align="center">
        <template slot-scope="scope">
          <span> {{ scope.row.role === 2 ? "系统管理员" : scope.row.role === 1 ? "设备管理员" : "设备查看员" }} </span>
        </template>
      </el-table-column>
      <el-table-column label="添加日期" width="180px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.createTime }}</span>
        </template>
      </el-table-column>
      <el-table-column label="登陆日期" width="180px" align="center">
        <template slot-scope="scope">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item v-permission="[2]" label="密码" prop="password">
          <el-input v-model="temp.password" />
        </el-form-item>
...
<script>
import { fetchUserList, fetchPv } from '@/api/article'
...
const calendarTypeOptions = [
  { key: '2', display_name: '系统管理员' },
  { key: '1', display_name: '设备管理员' },
  { key: '0', display_name: '设备查看员' }
]
  ...
  methods: {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['序号', '用户名字', '用户账号', '用户角色', '添加日期', '登陆日期']
        const filterVal = ['id', 'name', 'account', 'role', 'createTime', 'updateTime']
    ...
    formatJson(filterVal, jsonData) {
      return jsonData.map(v => filterVal.map(j => {
        if (j === 'timestamp') {
          ...
          return v[j] === 2 ? '系统管理员' : v[j] === 1 ? '设备管理员' : '设备查看员'
        } else {
          return v[j]
        }
      }))
